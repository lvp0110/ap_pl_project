...
const FALLBACK_FIELDS: CrmFormField[] = [
  ...
  field('number', 'planned_supply_year', 'Год начала поставки'),
  field('number', 'sale_probability', 'Вероятность поставки материалов'),
  field('list', 'ag_manager_id', 'Менеджер проекта от АГ'),
  field('list', 'sg_manager_id', 'Ответственный SG'),
  field('multiple_list', 'participant_ids', 'Участники проекта'),
  field('date', 'first_contact_date', 'Дата первого контакта c клиентом'),
  field('multiple_list', 'documentation_type_ids', 'Проделанная работа'),
  field('materials', 'materials', 'Материалы / решения'),
  {
    type: 'text_area',
